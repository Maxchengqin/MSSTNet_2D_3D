import warnings
from ops import msst_pre as msst
...
parser.add_argument('dataset', type=str, choices=['uav', 'pku', 'ucla', 'ntu60', 'ntu120'])
...
print('loading data ......')
data_loader = torch.utils.data.DataLoader(MSSTdata(dataroot=args.test_path, modality=args.modality, test_mode=True), batch_size=args.batchsize, shuffle=False, num_workers=args.workers * 2, pin_memory=True)
...
for i, (input, target) in enumerate(data_loader):
    ...
    (prec1, prec5), answer = accuracy(output.data, target, topk=(1, 5))
