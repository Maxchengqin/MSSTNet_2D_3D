from ops import msst_pre as network
import warnings
...
from ops.dataset_sk_3d import MSSTdata
# from datasetori import MSSTdata
from opts_sk import parser
from sklearn.metrics import confusion_matrix
best_prec1 = 0

modelstarttime = str(time.strftime("%Y%m%d%H"))
# file_handle = open('log' + str(time.strftime("%Y-%m-%d-%H-%M")) + parser.parse_args().model_name + '.txt', mode='w+')
file_handle = open('logs/'+'log' + str(time.strftime("%Y-%m-%d-%H-%M")) + '_' + parser.parse_args().model_name + '_' + parser.parse_args().modality + '.txt', mode='w+')
...
def main():
    ...
    valdata = MSSTdata(dataroot=args.data_root, modality=args.modality,  test_mode=True)
    ...
    traindata = MSSTdata(dataroot=args.data_root, modality=args.modality, test_mode=False)
...
def validate(val_loader, model, criterion, iter, logger=None):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        (prec1, prec5), prec_1_label = accuracy(output.data, target, topk=(1, 5))
        prec1_label_list.append(prec_1_label)
    ...
    pred_1 = torch.cat(prec1_label_list, 0)
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    filename = '_'.join((modelstarttime, args.model_name, args.modality.lower(), filename))
    torch.save(state, filename)
    if is_best:
        best_name = '_'.join((modelstarttime, args.model_name, args.modality.lower(), 'model_best.pth.tar'))
